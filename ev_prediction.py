...
Original file is located at
    https://colab.research.google.com/drive/1BHG8veY4ByEVeLuomT7-V6O0ibPVovjL
"""
...
# @title
df=pd.read_csv(r"/content/internship-EV-/3ae033f50fa345051652.csv")
...
# How this works:
# Q1 and Q3: The 25th and 75th percentiles of your data column.

# IQR (Interquartile Range): Q3 - Q1, measures middle 50% spread.

# Lower/Upper bounds: Define what counts as an outlier (anything below Q1 - 1.5IQR or above Q3 + 1.5IQR).
...
df['ev_total_pct_change_1'] = df['ev_total_pct_change_1'].replace([np.inf, -np.inf, np.nan], np.nan)
...
# Combine and plot (code from your latest image)
historical['Date'] = pd.to_datetime(historical[['year', 'month']].assign(day=1))
...
for source, group in combined.groupby('Source'):
    plt.plot(
        group['Date'], group['Cumulative EVs'],
...
for county in unique_counties:
    ...
    future_rows = []
    # Move forecast loop inside county loop
    latest_row = county_df.iloc[-1].copy() # Get the last row of the historical data for this county
    for _ in range(forecast_horizon):
        # Update month and year for the forecast
        next_date = pd.to_datetime(latest_row['year'] * 100 + latest_row['month'], format='%Y%m') + pd.DateOffset(months=1)
        y, m = next_date.year, next_date.month
        #numeric_date is not used in prediction features but good for tracking
        numeric_date = y * 12 + m
        months_since_start += 1

        # Calculate features for the next forecast step
        lag1, lag2, lag3 = historical_ev[-1], historical_ev[-2], historical_ev[-3]
        roll_mean = np.mean([lag1, lag2, lag3])
        pct_change_1 = (lag1 - lag2) / lag2 if lag2 != 0 else 0
        pct_change_3 = (lag1 - lag3) / lag3 if lag3 != 0 else 0
        ...
        # Update rolling histories with the predicted value for the next iteration
        historical_ev.append(pred)
        if len(historical_ev) > 6:
            historical_ev.pop(0)
        ...
        # Update latest_row date for the next forecast month
        latest_row['year'], latest_row['month'] = next_date.year, next_date.month


    # Combine historical and forecast data for this county
    forecast_df = pd.DataFrame(future_rows)
    combined_county_df = pd.concat([historical, forecast_df], ignore_index=True)

    # Append to the overall list
    all_combined.append(combined_county_df)

# Combine all counties
full_df = pd.concat(all_combined)

# Sort by date to ensure proper cumulative behavior across all counties
full_df = full_df.sort_values(["County", "Date"])

# Calculate cumulative EV count per county across the full combined data
full_df['Cumulative EVs'] = full_df.groupby('County')['Electric Vehicle (EV) Total'].cumsum()
...
for county, group in top_5_df.groupby('County'):
    # Plot both historical and forecast for each county in the same line
    plt.plot(group['Date'], group['Cumulative EVs'], label=county, marker='.')
